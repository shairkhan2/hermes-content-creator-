...
Checks: banned constructions, unfalsifiable attribution, hedging, word budget,
citation coverage, sentence rhythm, adverb density, repeated openers, restatement.

Usage:
    python3 lint_draft.py draft.md --pack pack.json --budget 95
    python3 lint_draft.py draft.md --pack pack.json --budget 95 --json
    python3 lint_draft.py draft.md --budget 95 --no-claims
...
def run_checks(text, claims, budget, tolerance, require_claims):
    findings = []
...
def main():
    ...
    findings, word_count, cited = run_checks(
        text, claims, args.budget, args.tolerance, not args.no_claims
    )
