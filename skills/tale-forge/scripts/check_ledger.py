...
Two modes. Research storytelling owes an answer to every question it raises. Fiction
may leave one deliberately open — but only when the ledger says so and says why.

Usage:
    python3 check_ledger.py ledger.json
    python3 check_ledger.py ledger.json --mode fiction --json
...
# Research storytelling owes the listener every answer it promised. Fiction does not:
# a question deliberately left open is the genre working, not a defect. Fiction mode
# permits that only when the ledger says so out loud.
...
def is_deliberately_open(q):
    """Marked unresolved, with a stated reason. Silence is not a mark."""
    return bool(q.get("unresolved")) and bool(str(q.get("unresolved_reason", "")).strip())


def check_questions(data, mode=MODE_RESEARCH):
        ...
        if q.get("unresolved"):
            if mode != MODE_FICTION:
                out.append(finding("orphan-question", SEVERITY_ERROR,
                                   f"{qid} is marked unresolved, which only {MODE_FICTION} "
                                   "mode permits — research storytelling owes an answer to "
                                   "every question it raises", where))
            elif not str(q.get("unresolved_reason", "")).strip():
                out.append(finding("unresolved-without-reason", SEVERITY_ERROR,
                                   f"{qid} is marked unresolved but gives no "
                                   "unresolved_reason — an unanswered question has to be a "
                                   "decision, not an oversight", where))
            else:
                out.append(finding("deliberately-open", SEVERITY_WARN,
                                   f"{qid} is left open on purpose: "
                                   f"{q.get('unresolved_reason')}", where))
        elif not isinstance(pays, int) or not 1 <= pays <= total:
            out.append(finding("orphan-question", SEVERITY_ERROR,
                               f"{qid} never pays off — pays_off_at must be an int in 1..{total}",
                               where))
        if not q.get("unresolved") and isinstance(opened, int) \
                and isinstance(pays, int) and pays <= opened:
...
def check_root(data, by_id, mode=MODE_RESEARCH):
    ...
    root = roots[0]
    if root.get("unresolved"):
        if mode != MODE_FICTION:
            out.append(finding("root-unresolved", SEVERITY_ERROR,
                               f"root {root.get('id')} is marked unresolved — research "
                               "storytelling must answer the question it opened"))
        else:
            out.append(finding("root-unresolved", SEVERITY_WARN,
                               f"root {root.get('id')} never resolves. Ambiguity is a "
                               "legitimate ending, but the story still has to land "
                               "something in the final beat or it just stops"))
        return out, root
...
def check_nesting(by_id):
    ...
    for qid, q in by_id.items():
        ...
        # A deliberately open question runs past the end by design, and a child of one
        # cannot outlive a parent that never closes.
        if q.get("unresolved") or parent.get("unresolved"):
            continue
...
def check_open_loops(data, by_id, root):
    ...
    for beat in range(1, total):
        open_subs = [
            ...
            and q["opened_at"] <= beat
            and (q.get("unresolved")
                 or (isinstance(q.get("pays_off_at"), int) and beat < q["pays_off_at"]))
        ]
        if not open_subs:
    ...
    for qid, q in by_id.items():
        ...
        carried = [
            ...
            and other["opened_at"] <= pays
            and (other.get("unresolved")
                 or (isinstance(other.get("pays_off_at"), int)
                     and pays < other["pays_off_at"]))
        ]
...
def run_checks(data, mode=MODE_RESEARCH):
    findings = check_shape(data)
    if findings:
        return findings

    findings += check_beats(data)
    q_findings, by_id = check_questions(data, mode)
    findings += q_findings
    findings += check_cycles(by_id)

    root_findings, root = check_root(data, by_id, mode)
...
def main():
    ...
    parser.add_argument("--mode", default=MODE_RESEARCH,
                        ...
                        help="fiction permits questions marked deliberately unresolved")
    args = parser.parse_args()

    data = load_ledger(args.ledger)
    mode = data.get("mode", args.mode)
    if mode not in (MODE_RESEARCH, MODE_FICTION):
        print(f"unknown mode {mode!r}", file=sys.stderr)
        sys.exit(2)
    findings = run_checks(data, mode)
    ...
    if args.as_json:
        ...
        if not findings:
            ...
            print(f"ledger ok ({mode}) — {qs} questions across {beats} beats, "
                  "chain unbroken")
        else:
